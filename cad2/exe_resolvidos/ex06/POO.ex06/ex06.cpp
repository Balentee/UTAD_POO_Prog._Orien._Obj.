...
//todos os metodos recebem a lista de funcionarios em referencia para poder permitir modifica��es se necess�rio
...
int main()
{
	...
	/*
	Este exercicio usa a classe vector em detrimento da abordagem comum de vectores est�tivos (ex: int J[10] )
	...
	e vai crescer e diminuir a medida que adicionamos ou removemos (com o metodo erase) posi��es nesse vector).
	como a classe vector � dinamica, ele possui um metodo para nos informar de quantos objectos est�o no vector nesse instante ( .size() )

	A dclara��o do vector tendo por base templates, o que nos permite ter um vector para o tipo de classe que quisermos.

	vector<int> L;
	vector<string> J;
	vector<Operario> M;

	ou Seja, vector<T>, onde T � o nome da classe ou tipo que queremos criar o objecto.
	*/
	   
	vector<Func*> Lista; //objcto da classe vector que permite armazenar apontadores do tipo Func*, o que permite alocar Operarios e Administrativos devido � rela��o de heran�a
	...
	while (true)
	{
		cout << endl << endl << " ------    Bem Vindo ao Sistema XPTO     -----  " << endl
			<< "\t 1 - Ler Ficheiro" << endl
			<< "\t 2 - Calcular e apresentar o ordenado de todos os funcion�rios." << endl
			<< "\t 3 - Pesquisar um funcion�rio pelo seu n�mero." << endl
			<< "\t 4 - Alterar o ordenado base de um funcion�rio, identificado pelo seu n�mero" << endl
			<< "\t 5 - Adicionar ao vector um Operario ou Administrativo" << endl
			<< "\t 6 - Apresentar o nome dos oper�rios que trabalham por turnos"<< endl
			<< "\t 7 - Eliminar do vetor um funcion�rio" << endl
...
void LerFicheiro(vector<Func*> &Lista)
{
	...
	do {	
		string tipo;
		getline(file, tipo, ':');  //no ficheiro o tipo de funcionario � dado pelo primeiro caracter separado por : da restante linha

		if (tipo == "O")
		{
			Operario *op = new Operario(); //alocar o espa�o para um operario
	...
	while (file.peek() != EOF); //repetimos at� chegar o caracter seguinte ser o fim do ficheiro.	
...
void CalcularOrdenados(vector<Func*> &Lista)
{
	//para todos os funcionarios na lista vamos apresentar a informa��o no ecra;
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		cout << Lista[i]->GetNum() << "\t"
			<< Lista[i]->GetNome() << "\t"
			<< Lista[i]->Calcula_Ordenado() << endl; //como o CalculaOrdenadao � um metodo virtual puro, ele vai chamar correctamente a implementa��o 
...
void PesquisaFuncionario(vector<Func*> &Lista)
{
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		if (Lista[i]->GetNum() == numero)
		{
			...
			{
				//se for um operario, ele vai converter neste if, e executa esta linha, caso n�o seja, ele n�o entra aqui, pois a convers�o resulta em NULL
				op->Show();
			}

			if (Administrativo *adm = dynamic_cast<Administrativo*>(Lista[i]))
			{
				//se for um Administrativo, ele vai converter neste if, e executa esta linha, caso n�o seja, ele n�o entra aqui, pois a convers�o resulta em NULL
	...
	cout << "N�o foi encontrado o utilizador" << endl;
...
void AlteraOrdenado(vector<Func*> &Lista)
{
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		if (Lista[i]->GetNum() == numero) //procuramos o funcionario na lista, e se encontrar
		{
			...
			float ord;
			cin >> ord;
			//como o metodo de alterar o ordenado pertence a classe Func n�o � necess�rio converter o objecto com o dynamic cast
	...
	cout << "N�o foi encontrado nenhum funcionario com o numero " << numero << endl;
...
void CriarFuncionario(vector<Func*> &Lista)
{
	...
	if (tipo == "O" || tipo == "o")
	{
		...
		op->ReadK(); //o metodo readK permite ler a informa��o a partir do teclado.
		Lista.push_back(op);
	}
...
void ApresentaTurnos(vector<Func*> &Lista)
{
	...
	{
		if (Operario *op = dynamic_cast<Operario*>(Lista[i])) // tentamos converter para operario, caso n�o seja possivel � porque nessa posi��o est� um administrativo
		{
...
void RemoveFuncionario(vector<Func*> &Lista)
{
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		...
		{
			//removemos do vector com o metodo erase, que recebe um "iterador" que aponta para a posi��o que queremos remover a partir do inicio.
...
void GuardarAdministrativos(vector<Func*> &Lista)
{
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		//como apenas queremos guardar administrativos, vamos tentar converter, se for possivel guardamos no ficheiro, se n�o for, 
		// � um operario, ent�o passamos a frente.
		if (Administrativo *adm = dynamic_cast<Administrativo*>(Lista[i]))
		{
			adm->SaveFile(file);
			file << endl;
...
void GuardarOperarios(vector<Func*> &Lista)
{
	...
	for (int i = 0; i < Lista.size(); i++)
	{
		//como apenas queremos guardar Operarios, vamos tentar converter, se for possivel guardamos no ficheiro, se n�o for, 
		// � um Administrativos, ent�o passamos a frente.
